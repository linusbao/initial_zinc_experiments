...
def load_zinc_subcount_dataset(name, sub_file, idx_list, root):    
    if name == "ZINC":
        train_data, val_data, test_data, _ = load_zinc_dataset(name, root)
    ...
    subcount_dim = subcount_dataset[0].pe.size()[1]
...
def load_zinc_homcount_dataset(name, hom_files, idx_list, root):    
    ...
    homcount_dim = homcount_dataset[0].pe.size()[1]
